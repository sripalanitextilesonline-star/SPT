...
export function VeloKeysForm() {
  ...
  const usageSnippet = useMemo(() => {
    ...
    return [
      ...
      "    `http://localhost:3000/api/velo/orders?since=${encodeURIComponent(since)}&limit=50`,",
  ...
  const productsUsageSnippet = useMemo(() => {
    ...
    return [
      ...
      "}",
      "",
      "// Preferred: upload image bytes to media.sripalanitextiles.com (not Vercel)",
      ...
      "}",
      "",
      "// Parallel uploads (industry standard) — do NOT use imageBase64",
      ...
      "});",
      "",
      "// 3) Category with image (same direct-upload path)",
      "const categoryImageId = await uploadProductImage(categoryFileFromCamera);",
      "const category = await callProductsApi('upsertCollection', `cat-${Date.now()}`, {",
      "  name: 'Festival Silk',",
      "  description: 'Festival collection',",
      "  featuredImageMediaId: categoryImageId,",
      "});",
      "",
      "// 4) Upload 4 photos in parallel, then product upsert with first media id",
      "const mediaIds = await uploadImagesParallel([file1, file2, file3, file4]);",
      ...
      "});",
      "",
      "// 5) Bulk upsert (prefer featuredImageMediaId; imageBase64 still works as fallback)",
      ...
      "});",
      "",
      "// 6) Delete product by external id",
      "await callProductsApi('delete', `delete-${Date.now()}`, {",
      "  externalProductId: 'APP-1001',",
      "});",
      "",
      "// 7) Delete category (batched; default 10 products per call)",
      "await callProductsApi('deleteCollection', `del-cat-${Date.now()}`, {",
