...
export async function sendOrderSuccessWhatsApp(
  ...
  const body: WhatsAppTemplateBody | WhatsAppTextBody = templateName
    ...
    : {
        ...
        type: "text",
        text: {
          body: `Hi ${params.customerName || "Customer"}, your order #${params.orderId} is confirmed. Amount paid: INR ${params.amount}. Thank you for shopping with Sri Palani Textiles.`,
        },
      };
    ...
    for (const mobile of config.sellerMobiles) {
      ...
      const res = await postWhatsAppMessage(
        ...
        config.accessToken,
        {
          ...
          to,
          type: "text",
          text: { body: `[Sri Palani Textiles ALERT]\n${message}` },
        },
      );
      if (res.sent) sentCount += 1;
    }
